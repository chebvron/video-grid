...
function App() {
  ...
  const noteInputRefs = useRef(new Map<string, HTMLInputElement | null>())
  ...
  useEffect(() => {
    ...
    setActiveNoteTarget((current) => {
      if (current && !allKeys.has(current.videoKey)) {
        return null
      }
      return current
    })
  ...
  const registerNoteInput = useCallback(
    (pointId: string) =>
      (node: HTMLInputElement | null) => {
        if (node) {
          noteInputRefs.current.set(pointId, node)
        } else {
          noteInputRefs.current.delete(pointId)
        }
      },
    [],
  )
  ...
  const handleRemoveVideo = useCallback((key: string) => {
    ...
    setActiveNoteTarget((current) => (current && current.videoKey === key ? null : current))
  }, [])
  ...
  const removePoint = useCallback(
    (videoKey: string, pointId: string) => {
      ...
      updateCopyFeedback(videoKey, '')
      setActiveNoteTarget((current) =>
        current && current.videoKey === videoKey && current.pointId === pointId ? null : current,
      )
    },
    [updateCopyFeedback],
  )
  ...
  const registerPoint = useCallback(
    (videoKey: string, rowIndex: number, columnIndex: number) =>
      (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        updateCopyFeedback(videoKey, '')
        setActiveNoteTarget({ videoKey, pointId: id })
      },
    [activeVideoKey, columns, rows, updateCopyFeedback],
  )
  ...
  useEffect(() => {
    ...
    const input = noteInputRefs.current.get(activeNoteTarget.pointId)
    if (input) {
  ...
  useEffect(() => {
    if (activeNoteTarget && activeNoteTarget.videoKey !== activeVideoKey) {
      setActiveNoteTarget(null)
    }
  }, [activeNoteTarget, activeVideoKey])
  ...
  const handleNoteFocus = useCallback(
    (videoKey: string, pointId: string) => () => {
      ...
      setActiveNoteTarget((current) => {
        if (current && current.videoKey === videoKey && current.pointId === pointId) {
          return current
        }
        return { videoKey, pointId }
      })
    },
    [activeVideoKey],
  )
                    ...
                    {video.points.map((point) => (
                      <li key={point.id} className="annotations__item">
                        ...
                        <input
                          ...
                          ref={registerNoteInput(point.id)}
                          value={point.note}
                          placeholder="Add a note"
                          onFocus={handleNoteFocus(video.key, point.id)}
                          onChange={(event) =>
                            updatePointNote(video.key, point.id, event.target.value)
                          }
                          onBlur={() => finishNoteEditing(video.key, point.id)}
                          onKeyDown={(event) => {
                            if (event.key === 'Enter') {
                              event.preventDefault()
